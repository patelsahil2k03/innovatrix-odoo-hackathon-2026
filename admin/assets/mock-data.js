/* ==========================================================================
   ...
   Shapes mirror the ERD in check.html (ROLES, USERS, VEHICLES, DRIVERS,
   TRIPS, MAINTENANCE_LOGS, FUEL_LOGS, EXPENSES, VEHICLE_HEALTH,
   VEHICLE_ANALYTICS, DRIVER_PERFORMANCE, AI_DISPATCH_SUGGESTIONS, ALERTS).
...
const DB = {
  ...
  notifications: [
    { title: "New maintenance ticket opened on MH-12-KL-7734", is_read: false, created_at: "2026-07-12T06:10:00Z" },
    { title: "Trip T-1001 dispatched successfully", is_read: false, created_at: "2026-07-12T03:05:00Z" },
    { title: "Monthly fleet analytics report is ready", is_read: true, created_at: "2026-07-10T08:00:00Z" },
  ],
...
DB.getVehicle = (id) => byId(DB.vehicles, id);
DB.getDriver = (id) => byId(DB.drivers, id);

DB.fmtMoney = (n) => n == null ? "—" : "₹" + Number(n).toLocaleString("en-IN");
DB.fmtDate = (iso) => {
  if(!iso) return "—";
...
/* ---- Geography (for the Fleet Map) --------------------------------------
   vehicle.region and trip source/destination_location are city names that
   key directly into this table — no separate geocoding step needed. */
...
DB.getVehicleLatLng = (vehicle) => {
  const base = DB.cityCoords[vehicle.region];
  if(!base) return null;
  const [dx, dy] = DB.jitter(vehicle.id);
  return [base[0] + dx, base[1] + dy];
};

// Last 7 days of fleet-wide revenue, for the Analytics trend chart.
DB.revenueTrend = [
